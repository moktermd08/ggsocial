...
| Scheduler | pm2 `ggsocial-scheduler` → ticks `/api/cron/publish` every 60s, and hourly `/api/cron/goals` (follower counts daily, goal re-plans on Mondays), `/api/cron/pages` and `/api/cron/playbook` (posting-time suggestions for the daily review) |
